...
@documento.get("/", response_description="Documentos listados", response_model=List[Documento])
async def obtener_documentos(token: str = Depends(esquema_oauth)):
    documentos = await conn["documentos"].find().to_list(1000)
    return documentos


@documento.get("/documentos/{documento_id}", response_description="Documento obtenido", response_model=Documento)
async def obtener_documento_por_id(documento_id: str, token: str = Depends(esquema_oauth)):
    if (documento := await conn["documentos"].find_one({"_id": documento_id})) is not None:
        return documento
...
@documento.get("/documentos/titulo/{titulo}", response_description="Documento obtenido", response_model=Documento)
...
@documento.post("/documentos/guardar", response_description="Documento creado", response_model=Documento)
async def guardar_documento(documento: Documento = Body(...), token: str = Depends(esquema_oauth)):
    ...
    documento_creado = await conn["documentos"].find_one({"_id": documento_insertado.inserted_id})
    return JSONResponse(status_code=status.HTTP_201_CREATED, content=documento_creado)


@documento.put("/documentos/actualizar/{documento_id}", response_description="Documento actualizado", response_model=Documento)
async def actualizar_documento(documento_id: str, actualizarDocumento: ActualizarDocumento = Body(...), token: str = Depends(esquema_oauth)):
    ...
    if len(actualizarDocumento) >= 1:
        update_result = await conn["documentos"].update_one({"_id": documento_id}, {"$set": actualizarDocumento})

        if update_result.modified_count == 1:
            if (
                documento_actualizado := await conn["documentos"].find_one({"_id": documento_id})
            ) is not None:
                return documento_actualizado

    if (documento_existente := await conn["documentos"].find_one({"_id": documento_id})) is not None:
